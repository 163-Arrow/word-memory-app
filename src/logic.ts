...
export function appendImportedWords(session: DailySession, wordIds: number[]): DailySession {
 const next = structuredClone(session); for (const wordId of wordIds) { if (!next.wordIds.includes(wordId)) { next.wordIds.push(wordId); next.progresses[wordId] = { wordId, streak: 0, qualified: false, correct: 0, wrong: 0 }; next.completed = false; } } return next;
}
